			...
			<>
				<Card className="mt-5">
					<CardHeader>
						...
						<CardDescription>
							Revise as informações abaixo. Se estiver tudo certo clique em
							criar cenário!
						</CardDescription>
					</CardHeader>
					<CardContent className="flex flex-col gap-2">
						...
						<div className="flex flex-col md:flex-row md:items-center gap-2">
							...
							<div className="flex flex-wrap items-center gap-3">
								...
								>
									...
									<PopoverAnswers
										message={
											formData.answers.isNotClickable === "firstAnswer" ? (
												<>
													Esta resposta <strong>não</strong> poderá ser clicada.
												</>
											) : (
								...
								>
									...
									<PopoverAnswers
										message={
											formData.answers.isNotClickable === "secondAnswer" ? (
												<>
													Esta resposta <strong>não</strong> poderá ser clicada.
												</>
											) : (
						...
						<div className="flex flex-col gap-2 mt-4">
							<Typography variant="strong">Confirmação:</Typography>
							...
							{formData.confirmation.confirmationType === "text" ? (
								<p className="py-1 px-2 border border-border rounded-md">
									{formData.confirmation.confirmationContent}
								</p>
							) : formData.confirmation.confirmationType === "image" ||
								formData.confirmation.confirmationType === "gif" ? (
								<img
									src={formData.confirmation.confirmationContent}
									width={420}
									height={420}
									className="h-auto w-full max-w-full mt-2 object-contain"
									alt="Confirmação do cenário"
								/>
							) : formData.confirmation.confirmationType === "video" ? (
								<YoutubeEmbed url={formData.confirmation.confirmationContent} />
							) : null}
						</div>
					</CardContent>
					<CardFooter className="flex items-center justify-between gap-5 w-full mt-10">
						<Button className="w-full" variant="outline" onClick={prev}>
							<>
						...
						>
							{isCreatingScenario ? (
								<>
									<Loader2 className="size-5 animate-spin" /> Criando cenário...
								</>
							) : (
								"Criar cenário"
							)}
